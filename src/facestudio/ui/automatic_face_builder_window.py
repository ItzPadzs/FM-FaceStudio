...
class CalibrationEditorDialog(QDialog):
    ...
    def __init__(self, record: Path, mode: str, parent=None) -> None:
        ...
        key = "landmarks" if mode == "geometry" else "anchors"
        self.points = self.payload[key]
        ...
        self.table = QTableWidget(len(self.points), 3)
        self.table.setHorizontalHeaderLabels(["Point", "X", "Y"])
...
class AutomaticFaceBuilderWindow(QMainWindow):
    ...
    def __init__(self, config: AppConfig, config_path: Path) -> None:
        ...
        self.photo: Path | None = None; self.dataset: Path | None = None
        self.assets: Path | None = None; self.profiles: Path | None = None
        self.workspace: Path | None = None; self.last_result: AutomaticBuildResult | None = None
    ...
    def _build_ui(self) -> None:
        ...
        version = QLabel("ALPHA 8.3.0\nAUTO CALIBRATION"); version.setObjectName("Muted"); sl.addWidget(version)
        sl.addSpacing(20)
        ...
        self.assets_label = QLabel("No donor texture folder selected")
        self.profile_label = QLabel("Profiles will be created inside the output folder")
        ...
        form.addRow("Output", self._row(self.output_label, self._button("Choose Output Folder", self.choose_workspace)))
        layout.addWidget(setup)

        content = QHBoxLayout()
        self.preview = self._preview_card(content, "Your Photograph", "Upload one clear front-facing photograph")
        self.result_preview = self._preview_card(content, "Automatic Result", "The refined donor texture will appear here")
        ...
        actions = QHBoxLayout()
        self.build_button = QPushButton("✦  Build Face Automatically"); self.build_button.setMinimumHeight(48); self.build_button.clicked.connect(self.build)
        ...
        self.rebuild_button = QPushButton("Rebuild With Changes"); self.rebuild_button.setEnabled(False); self.rebuild_button.clicked.connect(self.build)
        actions.addWidget(self.build_button, 1); actions.addWidget(self.geometry_button); actions.addWidget(self.uv_button); actions.addWidget(self.rebuild_button)
    ...
    def choose_photo(self) -> None:
        ...
        self.photo = Path(filename); self.photo_label.setText(filename); self._set_preview(self.preview, filename)
    ...
    def build(self) -> None:
        if not all((self.photo, self.dataset, self.assets, self.workspace)):
            QMessageBox.information(self, "Quick build setup", "Choose the photograph, geometry dataset, donor asset folder and output folder first.")
            return
        ...
        try:
            ...
            result = self.service.build(AutomaticBuildInputs(self.photo, self.dataset, self.assets, self.workspace, self.profiles))
            self.last_result = result; self.progress.setValue(100)
            validation = result.integrated.validation_result
            uv_text = "generated automatically" if result.generated_uv_profile else "reused"
            ...
            self.progress.setValue(0); self.status.setText("Automatic build stopped. Check the selected resources or open Advanced Workspace.")
            QMessageBox.critical(self, "Automatic build failed", str(exc))
        finally:
            self.build_button.setEnabled(True)

    def tune_geometry(self) -> None:
        if self.last_result and CalibrationEditorDialog(self.last_result.geometry_record, "geometry", self).exec():
            self.status.setText("Geometry fine tuning saved. Press Rebuild With Changes to apply it.")

    def tune_uv(self) -> None:
        if self.last_result and CalibrationEditorDialog(self.last_result.uv_record, "uv", self).exec():
            self.status.setText("UV fine tuning saved. Press Rebuild With Changes to apply it.")
